...
class ExportFile(models.AbstractModel):
    ...
    def _get_bank_accounts(self):
        ...
        if ICPSudo.get_param('payroll.main.bank.account.id'):
            if ICPSudo.get_param('payroll.main.bank.account.id') != '':
                company_account_id = int(ICPSudo.get_param('payroll.main.bank.account.id'))
        ...
        if ICPSudo.get_param('payroll.cash.bank.account.id'):
            if ICPSudo.get_param('payroll.cash.bank.account.id') != '':
                company_account_id = int(ICPSudo.get_param('payroll.cash.bank.account.id'))
                company_bank_account = self.env['res.partner.bank'].sudo().search([('id', '=', company_account_id)],
                                                                                  limit=1)
                if (len(company_bank_account) > 0):
                    accounts['Cash'] = company_bank_account[0]

        if ICPSudo.get_param('payroll.bank.account.ids'):
            if ICPSudo.get_param('payroll.bank.account.ids') != '':
                for id in ICPSudo.get_param('payroll.bank.account.ids').split(','):
