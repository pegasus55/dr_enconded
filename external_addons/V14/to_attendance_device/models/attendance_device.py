...
class AttendanceDevice(models.Model):
    ...
    port = fields.Integer(string="Port", required=True, default=4370, tracking=True, readonly=False, states={'confirmed': [('readonly', True)],
                                                                                                             'cancelled': [('readonly', True)]})
    timeout = fields.Integer(string='Timeout', default=20, required=True, help='Maximum time in seconds to wait for respond from the device', tracking=True,
                             readonly=False, states={'confirmed': [('readonly', True)],
                                                     'cancelled': [('readonly', True)]})
    ...
    def clearAttendance(self):
        ...
        try:
            ...
            raise ValidationError(_('Could not clear attendance data from the device %s') % (self.display_name,))
        finally:
            self.enableDevice()
            self.disconnect()
