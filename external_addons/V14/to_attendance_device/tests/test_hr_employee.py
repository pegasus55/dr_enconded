...
class TestHrEmployee(Common):
    ...
    def test_01_compute_total_finger_template_records(self):
        ...
        self.env['finger.template'].create({
            'fid': 111,
            'valid': 111,
            'employee_id': self.hr_empployee.id,
            })
        self.hr_empployee._compute_total_finger_template_records()
        self.assertEqual(self.hr_empployee.total_finger_template_records, 2)
