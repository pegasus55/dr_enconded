...
class Company(models.Model):
    ...
    def _get_default_favicon(self, original=False):
        # use viindoo's favicon if it exists in branding_module/static/src/img/favicon.ico
        branding_module = _get_branding_module()
        for adp in odoo.addons.__path__:
            if viin_brand_manifest.get('installable', False):
                viindoo_favicon_path = get_resource_path(branding_module, 'static/src/img/favicon.ico')
